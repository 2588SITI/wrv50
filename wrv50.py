...
# --- Logic Helpers ---
def clean_id(s):
    """Cleans Signal IDs to ensure matching (e.g., 'S-12' becomes 'S12')"""
...
def process_files(rtis_file, dlog_file, sig_file):
    with st.spinner("Synchronizing RTIS & Datalogger Tracks..."):
        try:
            # 1. Load Mapping (Looks at Column 7/Index 6)
            sig_map = load_data_smart(sig_file)
            if sig_map is None: return
            
            up_signals = set()
            col_idx = 6 if len(sig_map.columns) > 6 else 0
            for s in sig_map.iloc[:, col_idx].dropna():
                up_signals.add(clean_id(s))
            
            # SIDEBAR DEBUGGER
            st.sidebar.info(f"Loaded {len(up_signals)} Signals from Mapping.")
...
# --- Footer Section ---
st.markdown("---")
st.markdown(f"<div style='text-align: center; color: grey;'><b>ADEE TRO BL</b> | Indian Railways</div>", unsafe_allow_html=True)
